...
public  class ComPort {
	...
	protected void onDataReceived(byte[] buffer, int size) {
		...
		if (readBuffer.contains("FE")) {
			...
			readBuffer = "";
			Log.e("~~~~","开头可能就不正确 readBuffer = kong ");
		}
			...
			if (readBuffer.length()>= len * 2) {
				...
				if (sBegin.equals("FE")) {
					...
					if (check_com_data_string(msgContent, len * 2)) {
						...
						//指令不正确
						Log.e("指令不正确", msgContent + "***" + readBuffer);
						readBuffer = readBuffer.substring(2);
						if (readBuffer.contains("FE")) {
							readBuffer = readBuffer.substring(readBuffer.indexOf("FE"));
						} else {
							readBuffer = "";
							Log.e("~~~~","指令不正确 不包含FE readBuffer = kong ");
						}
					}
				} else
				{
					...
					if (readBuffer.contains("FE")) {
						...
						readBuffer = "";
						Log.e("~~~~","开头不正确 目前不包含FE readBuffer = kong ");
					}
				}
			}
			else
